...
public class Enemy : PoolObject<EnemySpawner, Enemy, Vector2>
{
    ...
    {
        enemyAI = instance.GetComponent<EnemyAI>();
        enemyController = instance.GetComponent<EnemyController>();
        healthBehaviour= instance.GetComponent<HealthBehaviour>();


        enemyAI.walkPoint = enemyAI.gameObject.transform;
        enemyController.ControlledByPool = true;
        enemyController.OnDie += ReturnToPoolEvent;
    ...
    public override void WakeUp(Vector2 info)
    {
        ...
        enemyAI.EnableMovements();
        healthBehaviour.HealthToMax();
    }
    ...
    protected void ReturnToPoolEvent(BaseController enemyController)
    {
